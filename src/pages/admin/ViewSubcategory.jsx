...
const ViewSubcategories = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError('Failed to load subcategories or categories');
  ...
  const openEditModal = (subcategory) => {
    ...
    setFormData({
      ...
      categoryId: subcategory.categoryId?._id || subcategory.categoryId, // Handle populated vs. non-populated categoryId
    });
    setImagePreview(subcategory.imagePath ? `http://localhost:3000${subcategory.imagePath}` : null);
    setIsModalOpen(true);
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      data.append('categoryId', formData.categoryId); // Backend expects categoryId
      if (image) {
        data.append('image', image);
      }

      const response = await axiosInstance.put(`/subcategories/${editSubcategory._id}`, data, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      // Update the subcategories list with the updated subcategory
      setSubcategories(subcategories.map(subcat => (subcat._id === editSubcategory._id ? response.data : subcat)));
  ...
  return (
    <Container>
      ...
      {subcategories.length === 0 ? (
        <NoData>No subcategories found.</NoData>
      ) : (
        <Table>
          <thead>
            <tr>
              ...
              <Tr key={subcategory._id}>
                ...
                <Td>
                  {subcategory.imagePath ? (
                    <SubcategoryImage
                      src={`http://localhost:3000${subcategory.imagePath}`}
                ...
                <Td>
                  <ActionButton onClick={() => openEditModal(subcategory)}>
                    <FaEdit /> Edit
                  </ActionButton>
                  <ActionButton delete onClick={() => handleDelete(subcategory._id)}>
                    <FaTrash /> Delete
                  </ActionButton>
                </Td>
              </Tr>
        ...
        <ModalOverlay>
          <ModalContent>
            ...
            <ModalForm onSubmit={handleEditSubmit}>
              ...
              <SubmitButton type="submit">Update Subcategory</SubmitButton>
            </ModalForm>
          </ModalContent>
        </ModalOverlay>
      )}
